...
from lib.st.cached import db_manager, prompts_registry
    ...
    if "session_chain" not in st.session_state:
        with st.spinner("Building..."):
            ...
            st.session_state["session_chain"] = get_main_chain_stream(
                prompts_registry(), get_chain_sink()
            )
...
def main():
    ...
    for mi, message in enumerate(st.session_state.messages):
        with st.chat_message(message["role"]):
            ...
            if message["role"]=='ai' and mi>0:
                raw, md = st.tabs(["Raw","Rendered"])
                raw.code(msg_content, language="html")
                md.markdown(msg_content, unsafe_allow_html=True)

                copy_button_raw = st.button("Copy")

                if copy_button_raw:
                    # Create a JavaScript function to copy the text to the clipboard
                    js_code = """
                        ...
                        """
                    # Call the JavaScript function using st.markdown
                    st.markdown(f"""
                        <script>
                            {js_code}
                            copyTextToClipboard("{msg_content}");
                        </script>
